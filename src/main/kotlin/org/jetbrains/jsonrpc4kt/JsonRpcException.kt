    ...
    companion object {
        ...
        fun indicatesStreamClosed(thr: Throwable?): Boolean {
            return (thr is InterruptedIOException
                    || thr is ClosedChannelException || thr is IOException && ("Stream closed" == thr.message || "Pipe closed" == thr.message)) || thr is SocketException && ("Connection reset" == thr.message || "Socket closed" == thr.message || "Broken pipe (Write failed)" == thr.message) || thr is org.jetbrains.jsonrpc4kt.JsonRpcException && org.jetbrains.jsonrpc4kt.JsonRpcException.Companion.indicatesStreamClosed(
                thr.cause
            )
        }
    }
